...
)
{
    ...
    LE_ASSERT(le_gnss_PackConstellationBitMask( &_msgBufPtr, &_msgBufSize,
                                                  constellationMask ));
...
)
{
    ...
    if (constellationMaskPtr &&
        (!le_gnss_UnpackConstellationBitMask( &_msgBufPtr, &_msgBufSize,
...
)
{
    ...
    // Pack the input parameters
    LE_ASSERT(le_gnss_PackConstellation( &_msgBufPtr, &_msgBufSize,
                                                  satConstellation ));
    LE_ASSERT(le_gnss_PackConstellationArea( &_msgBufPtr, &_msgBufSize,
                                                  constellationArea ));
...
)
{
    ...
    if (constellationAreaPtr &&
        (!le_gnss_UnpackConstellationArea( &_msgBufPtr, &_msgBufSize,
...
)
{
    ...
    // Unpack any "out" parameters
    if (statePtr &&
        (!le_gnss_UnpackFixState( &_msgBufPtr, &_msgBufSize,
                                               statePtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Unpack the result first
    if (!le_gnss_UnpackSbasConstellationCategory( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    LE_ASSERT(le_pack_PackReference( &_msgBufPtr, &_msgBufSize,
                                                  positionSampleRef ));
    LE_ASSERT(le_gnss_PackDopType( &_msgBufPtr, &_msgBufSize,
                                                  dopType ));
...
)
{
    ...
    // Pack the input parameters
    LE_ASSERT(le_gnss_PackAssistedMode( &_msgBufPtr, &_msgBufSize,
                                                  assistedMode ));
...
)
{
    ...
    // Unpack any "out" parameters
    if (assistedModePtr &&
        (!le_gnss_UnpackAssistedMode( &_msgBufPtr, &_msgBufSize,
                                               assistedModePtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Pack the input parameters
    LE_ASSERT(le_gnss_PackNmeaBitMask( &_msgBufPtr, &_msgBufSize,
                                                  nmeaMask ));
...
)
{
    ...
    // Unpack any "out" parameters
    if (nmeaMaskPtrPtr &&
        (!le_gnss_UnpackNmeaBitMask( &_msgBufPtr, &_msgBufSize,
                                               nmeaMaskPtrPtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Unpack the result first
    if (!le_gnss_UnpackState( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Pack the input parameters
    LE_ASSERT(le_gnss_PackResolution( &_msgBufPtr, &_msgBufSize,
                                                  resolution ));
...
)
{
    ...
    // Pack the input parameters
    LE_ASSERT(le_gnss_PackDataType( &_msgBufPtr, &_msgBufSize,
                                                  dataType ));
    LE_ASSERT(le_gnss_PackResolution( &_msgBufPtr, &_msgBufSize,
                                                  resolution ));
...
)
{
    ...
    // Pack the input parameters
    LE_ASSERT(le_gnss_PackCoordinateSystem( &_msgBufPtr, &_msgBufSize,
                                                  coordinateSrc ));
    LE_ASSERT(le_gnss_PackCoordinateSystem( &_msgBufPtr, &_msgBufSize,
                                                  coordinateDst ));
    LE_ASSERT(le_gnss_PackLocationDataType( &_msgBufPtr, &_msgBufSize,
                                                  locationDataType ));
